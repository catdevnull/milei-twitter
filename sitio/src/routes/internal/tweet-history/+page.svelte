...
{#if !data.authorized}
  <main class="mx-auto flex min-h-screen max-w-sm items-center px-6">
    <form method="POST" action="?/login" class="w-full space-y-4">
      <h1 class="text-2xl font-semibold">Tweet history</h1>
      <label class="block">
        <span class="mb-1 block text-sm">Scraper API token</span>
        <input
          name="token"
          type="password"
          required
      ...
      <button class="rounded-md bg-black px-4 py-2 text-white">Open</button>
    </form>
  </main>
{:else}
  <main class="mx-auto max-w-7xl space-y-8 px-4 py-8 sm:px-6">
    <header class="flex items-center justify-between gap-4">
      ...
      <form method="POST" action="?/logout">
        <button class="rounded-md border px-3 py-2 text-sm">Log out</button>
      </form>
    </header>
