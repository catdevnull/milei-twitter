...
async function readSqliteData(dbPath: string): Promise<SqliteData> {
  ...
  try {
    ...
    const scraperTokens = db.prepare(`
      SELECT id, token 
  ...
  try {
    ...
    if (data.scraps.length > 0) {
      ...
      for (let i = 0; i < scrapsData.length; i += batchSize) {
        ...
        console.log(`  ✓ Migrated scraps batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(scrapsData.length / batchSize)} (${batch.length} items)`);
      }
    ...
    if (data.historicLikedTweets.length > 0) {
      ...
      for (let i = 0; i < historicData.length; i += batchSize) {
        ...
        console.log(`  ✓ Migrated historic tweets batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(historicData.length / batchSize)} (${batch.length} items)`);
      }
      console.log(`✓ Migrated ${historicData.length} historic liked tweets total`);
    }
    ...
    if (data.likedTweets.length > 0) {
      ...
      for (let i = 0; i < likedData.length; i += batchSize) {
        ...
        console.log(`  ✓ Migrated liked tweets batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(likedData.length / batchSize)} (${batch.length} items)`);
      }
    ...
    if (data.retweets.length > 0) {
      ...
      for (let i = 0; i < retweetsData.length; i += batchSize) {
        ...
        console.log(`  ✓ Migrated retweets batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(retweetsData.length / batchSize)} (${batch.length} items)`);
      }
    ...
    if (data.tweets.length > 0) {
      ...
      for (let i = 0; i < tweetsData.length; i += batchSize) {
        ...
        console.log(`  ✓ Migrated tweets batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(tweetsData.length / batchSize)} (${batch.length} items)`);
      }
...
async function verifyMigration() {
  ...
  try {
    const counts = await Promise.all([
      db.select().from(schema.likedTweets).then(rows => rows.length),
      db.select().from(schema.historicLikedTweets).then(rows => rows.length),
      db.select().from(schema.retweets).then(rows => rows.length),
      db.select().from(schema.tweets).then(rows => rows.length),
      db.select().from(schema.scraps).then(rows => rows.length),
      db.select().from(schema.cuentas).then(rows => rows.length),
      db.select().from(schema.scraperTokens).then(rows => rows.length)
    ]);
